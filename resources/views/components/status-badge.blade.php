...
<span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold border {{ $current['class'] }} {{ $current['pulse'] ? 'relative overflow-hidden' : '' }} transition-all duration-200 hover:shadow-md hover:scale-105">
    @if($current['pulse'])
        <span class="absolute inset-0 bg-current opacity-5 animate-pulse rounded-full"></span>
    @endif
    <span class="relative flex items-center">
        <span class="inline-flex items-center justify-center w-5 h-5 rounded-full {{ $current['iconBg'] }} mr-1.5">
            {!! $current['icon'] !!}
        </span>
        {{ $current['label'] }}
    </span>
