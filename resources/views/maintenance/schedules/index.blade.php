        ...
        <div class="flex items-center justify-between">
            ...
            @if(auth()->user()->hasRole(['admin', 'manager', 'pelapor']))
                <div class="flex items-center gap-2">
                    <form action="{{ route('maintenance.schedules.generate-tasks') }}" method="POST" class="inline">
                        @csrf
                    ...
                    <a href="{{ route('maintenance.schedules.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-600 text-white text-sm font-medium rounded-xl hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg">
                        <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-100 dark:border-slate-700 shadow-sm">
                    ...
                    <p class="text-2xl font-bold text-blue-600">{{ $stats['daily'] }}</p>
                </div>
                <div class="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-100 dark:border-slate-700 shadow-sm">
                    <p class="text-xs text-purple-600">Weekly</p>
                    <p class="text-2xl font-bold text-purple-600">{{ $stats['weekly'] }}</p>
                </div>
                <div class="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-100 dark:border-slate-700 shadow-sm">
                    <p class="text-xs text-emerald-600">Monthly</p>
                    <p class="text-2xl font-bold text-emerald-600">{{ $stats['monthly'] }}</p>
                </div>
                <div class="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-100 dark:border-slate-700 shadow-sm">
            ...
            <div class="mb-4">
                <a href="{{ route('maintenance.tasks.index') }}" class="inline-flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 transition-colors">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                            @forelse($schedules as $schedule)
                                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                                    <td class="px-6 py-4">
                                        <p class="font-medium text-gray-900 dark:text-white">{{ $schedule->nama_tugas }}</p>
                                        <p class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ $schedule->deskripsi }}</p>
                                    </td>
                                    <td class="px-6 py-4">
                                        @php
                                            $freqColors = [
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $freqColors[$schedule->frekuensi] }}">
                                            {{ $freqLabels[$schedule->frekuensi] }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-gray-700 dark:text-slate-300">{{ $schedule->lokasi }}</td>
                                    <td class="px-6 py-4 text-gray-700 dark:text-slate-300">
                                        {{ $schedule->teknisi->name ?? 'Belum ditugaskan' }}
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="text-sm text-gray-600 dark:text-gray-400">
                                            {{ $schedule->checklists->count() }} item
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('maintenance.schedules.show', $schedule) }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            @if(auth()->user()->hasRole(['admin', 'manager', 'pelapor']))
                                                <a href="{{ route('maintenance.schedules.edit', $schedule) }}" class="text-amber-600 hover:text-amber-800 dark:text-amber-400 dark:hover:text-amber-300">
                                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                    </svg>
                                                </a>
                                                <form action="{{ route('maintenance.schedules.destroy', $schedule) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus jadwal ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    <td colspan="7" class="px-6 py-16 text-center">
                                        <div class="relative">
                                            ...
                                            <div class="relative">
                                                ...
                                                @if(auth()->user()->hasRole(['admin', 'manager', 'pelapor']))
                                                    <a href="{{ route('maintenance.schedules.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-xl hover:bg-indigo-700 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5">
